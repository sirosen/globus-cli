...
*transfer* creates an asynchronous task that copies files and/or directories
between endpoints. 

The *transfer* command has two modes. Single target, which transfers one file
or one directory, and batched, which takes in several lines to transfer 
multiple files or directories. See "Batch Input" below for more information.

The *transfer* command will always place the dest files in a
...
include::include/task_submission_options[]
